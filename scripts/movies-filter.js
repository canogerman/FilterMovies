...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function filterMovies(users, movies, userId, fromDate, toDate, rate) {
    // Perform the triple filter of movies by: userId, by watched (in years), and by rate.
    const filteredMovies = movies.filter((movie) => {
      const watchedYear = new Date(movie.watched).getFullYear();
      return (
        (userId == 0 || movie.userId == userId) &&
        watchedYear >= fromDate &&
        watchedYear <= toDate &&
        movie.rate >= rate
      );
    });

    // Get the userIds from filteredMovies
    const userIds = filteredMovies.map((movie) => movie.userId);

    // Filter the users that have the userIds from filteredMovies
    const filteredUsers = users.filter((user) => userIds.includes(user.id));

    // Combine the results and obtain the requested structure.
    const combinedArray = filteredMovies.map((movie) => {
      const user = filteredUsers.find((user) => user.id === movie.userId);
      return {
        id: user.id,
  ...
  filterButton.addEventListener("click", function () {
    ...
    const fromDate = parseInt(fromDateInput.value);
    const toDate = parseInt(toDateInput.value);
    const rate = parseInt(rateInput.value);

    // Call the filterMovies function passing the selected parameters
    ...
    // Show the results in the container
    displayResults(filteredMovies, resultsContainer);
  });
  ...
  function displayResults(movies, container) {
    ...
    counterMovies.textContent = `Number of movies founded: ${movies.length}`;
    counterMovies.classList.add("counter");
    container.appendChild(counterMovies);
